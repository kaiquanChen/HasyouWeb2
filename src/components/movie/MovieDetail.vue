    ...
    <div id="movie-detail" v-title data-title="测试title">
        <div class="left">
            <h1 class="movie-title">{{data.title}}
                <span class="movie-year" v-if="data.year && data.year.length > 0">({{data.year}})</span>
            </h1>
            <div class="content">
                <div class="avatar">
            ...
            <div class="movie-blooper">
                <h3>{{data.title}}的主题曲 · · · · · ·
                    ...
                    <div class="video-list">
                        <MovieVideo class="video-item" :source="source" v-for="(source, index) in sources" :key="source.src" v-if="index < 3"/>
                    </div>
                </h3>
            </div>
